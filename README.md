...
There is no separate Express service, ORM, Redis cache, WebSocket layer, real-money payment system, or microservice architecture in the MVP. The approved `VB01` Polar Sandbox per-venue entitlement is locally accepted across its SDK/database, owner checkout, webhook/enforcement, deadline, and offline-fixture paths; hosted/demo acceptance remains pending. It is not a real-payment system. The expected course scale is better served by a clear Next.js backend, indexed PostgreSQL queries, scheduled sports-data synchronization, and cursor pagination.
...
The current unmerged attendance-rediscovery correction passes the complete local acceptance command with 943 Vitest assertions and one skipped live-model test, all 1,685 pgTAP assertions, schema lint, generated-type drift, the production build, all 32 Playwright journeys, the secret audit, and diff hygiene. Its database regressions prove that an event stays out of Explore and general Ask while attendance is current, returns after the controlled leave transition while history remains, remains rediscoverable after an accepted direct invitation, and stays excluded when a new invitation is pending.

The current unmerged account-erasure revision contains the new migration, Server Action, Account Security Danger zone, browser-state cleanup, purpose-bound auth-link handling, truthful password-change failure recovery, and lifecycle tests. Its fresh complete local acceptance run passed 1,057 unit/component assertions with one live-model test skipped, all 1,746 database/RLS assertions, the production build, all 35 Playwright journeys, and the repository security/diff gates. This is not a production claim: the account-erasure migration has not been deployed, so hosted deletion acceptance remains pending.
