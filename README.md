...
The current full Fan-navigation follow-up explicitly warms every non-current primary Fan destination in Next.js's private per-tab Router Cache, including Home and Ask Huddle, while leaving Venue and billing routes outside this full-prefetch policy and persisting no account data. It also replaces Fan Home's five database round trips with one authenticated, actor-scoped read projection. Browser coverage proves warmed desktop and mobile clicks start no new destination RSC request; database coverage proves the projection's authorization and cross-Fan isolation. [Dated acceptance evidence](./docs/evidence/submission-hardening/ACCEPTANCE.md) keeps local verification separate from publication and production measurements.
