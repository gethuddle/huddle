...
Submission hardening passed local acceptance on 4 September: full-app audit corrections, loading/navigation work, generated venue Huddle URLs with optional settings edits, live handle/URL availability, secure account email changes, complete fixture-event pagination, and correctly loaded local map workers. Results: 1,482 unit/component tests, 2,606 database assertions, and 41 browser journeys pass. These changes are not yet published or deployed; [dated acceptance evidence](./docs/evidence/submission-hardening/ACCEPTANCE.md) separates verified local behavior from the remaining hosted and presentation checks.
