...
The merged full Fan-navigation follow-up explicitly warms every non-current primary Fan destination in Next.js's private per-tab Router Cache, including Home and Ask Huddle, while leaving Venue and billing routes outside this full-prefetch policy and persisting no account data. It also replaces Fan Home's five database round trips with one authenticated, actor-scoped read projection. Production tracing confirms that settled Home, Explore, Ask Huddle, My Huddle, and People clicks start no destination RSC request; two hard Home samples loaded in 697–791 ms. The deployed Explore follow-up replaces repeated authorized geospatial scans and a separate enrichment wave with one private no-store RPC. Comparable signed-in production discovery fell from 3.09 seconds to 333 ms; the six-request median was 647 ms, and warm cached results appeared in 214–217 ms while refreshing in the background. [Dated acceptance evidence](./docs/evidence/submission-hardening/ACCEPTANCE.md) records the exact gates, deployment, measurements, and claim boundaries.

The 5 September B13 technical-lock audit found and corrected a signed-out My Huddle failure, missing tablet-width public navigation, an overbroad production sports-host assertion, and a verification-journey navigation race. It also repaired one dead evidence link and made fresh-clone runtime selection explicit. Exact-source acceptance passed all 1,530 application tests, 2,667 database assertions, and 42 browser journeys, plus the production build, schema/type, security, dependency, and diff gates. [Issue #66](https://github.com/gethuddle/huddle/issues/66) tracks exact-head CI, deployment, and final production smoke. The course no longer requires a presentation; ignored submission documents remain local for a separate session.
...
There is no separate Express service, ORM, Redis cache, WebSocket layer, real-money payment system, or microservice architecture in the MVP. The approved `VB01` Polar Sandbox per-venue entitlement is locally accepted across its SDK/database, owner checkout, webhook/enforcement, deadline, and offline-fixture paths; its authorized hosted happy path is verified. It is not a real-payment system. The expected course scale is better served by a clear Next.js backend, indexed PostgreSQL queries, scheduled sports-data synchronization, and cursor pagination.
...
Install [fnm](https://github.com/Schniz/fnm) (or another version manager that reads `.node-version`), select the repository-pinned runtime, then install dependencies and start the local stack. With fnm in zsh:
...
The accepted B12 local run passed 403 Vitest/unit/component tests, 975 pgTAP assertions, the generated-type check, the production build, and all 17 Playwright journeys. Current post-B12 acceptance evidence is maintained under [`docs/evidence/`](./docs/evidence/). Local hand-in materials are intentionally excluded from the public repository.
