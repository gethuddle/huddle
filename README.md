...
The last complete merged verification passed 905 Vitest assertions with the one live-model test skipped, all 1,681 pgTAP assertions, generated-type drift, schema lint, production build, and all 31 Playwright journeys. On 2 September 2026, the credentialed 42-case evaluation passed every core, privacy, unsupported-scope, date-boundary, and supported-intent gate with prompt `ai01-v5`, including exact next-weekday and named-place queries. The checked-in corpus now contains 46 cases so named months, single dates, bare weekdays, and the date-free default remain explicit regression traits; those four deterministic additions do not require another model call. AI01 merged through PR #46 and was explicitly enabled in production; checked-in environment examples remain safely disabled by default.

The current unmerged follow-up branch passes the complete local acceptance command with 938 Vitest assertions and one skipped live-model test, all 1,681 pgTAP assertions, schema lint, generated-type drift, the production build, all 31 Playwright journeys, the secret audit, and diff hygiene. Its browser coverage includes the viewport-bounded mobile Ask transcript, automatic Jerusalem search, exact weekday and named-month interpretation, cleared state after navigation, and the revised Fan/Venue workspace navigation.
