...
The current unmerged follow-up branch passes the complete local acceptance command with 943 Vitest assertions and one skipped live-model test, all 1,681 pgTAP assertions, schema lint, generated-type drift, the production build, all 32 Playwright journeys, the secret audit, and diff hygiene. Its browser coverage includes the viewport-bounded mobile Ask transcript, automatic Jerusalem search, exact weekday and named-month interpretation, separately bordered multi-result tickets, cleared state after navigation, and the revised Fan/Venue workspace navigation.
