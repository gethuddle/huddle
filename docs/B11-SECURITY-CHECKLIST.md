...
| `/api/events/[eventId]/calendar.ics` | read-only `GET`; UUID route input | every venue-hosted and participant-specific output is authorized, audited, `private, no-store`; an active public venue calendar cannot outlive an entitlement change |
