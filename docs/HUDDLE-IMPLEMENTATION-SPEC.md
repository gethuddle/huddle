...
Venue Calendar and Events are owner/admin-only, server-filtered, bounded 20-row pages across retained lifecycle history. Their status filters apply before pagination, and an elapsed stored `published` event is shown as `completed` without destructively rewriting the event row. Event detail/edit return navigation may preserve only the current Venue's validated Calendar/Events status and page state.
...
**Operational evidence updated 4 September 2026:** the separately authorized guarded production Auth configuration apply and its exact `npm run auth:config:check` passed. The account-erasure migration and matching application code were subsequently deployed with verified production migration parity. Destructive deletion of a real production account remains excluded from routine acceptance; local lifecycle coverage is the primary evidence for that irreversible path.
